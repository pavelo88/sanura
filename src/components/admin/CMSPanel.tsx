  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(doc(db, 'settings', 'site-content'), (docSnap) => {
      if (docSnap.exists()) {
        ...
        setConfig({ heroTitle: '', heroSubtitle: '', doctorTitle: '', doctorQuote: '', doctorImage: '' });
      }
      setIsLoading(false);
    });
    return () => unsubscribe();
  ...
  return (
    <div className="space-y-8 relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* PORTADA HERO */}
        <div className={`bg-[#121A21] p-6 lg:p-8 rounded-2xl border border-[#1F2E3A] space-y-5 shadow-lg transition-opacity duration-500 ${isLoading ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder={isLoading ? "Cargando..." : "Ej: Belleza Médica con Rigor"}
            ...
            <div>
              ...
              <textarea
                ...
                placeholder={isLoading ? "Cargando..." : "Descripción breve del valor único"}
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder={isLoading ? "Cargando..." : "Ej: Dra. Natalia Vitali"}
                className="w-full bg-[#090D10] border border-[#1F2E3A] focus:border-[#5BC0BE] focus:ring-1 focus:ring-[#5BC0BE] p-3 text-sm text-white rounded-lg outline-none transition-all placeholder:text-white/20"
              />
            </div>
            <div>
              ...
              <textarea
                ...
                placeholder={isLoading ? "Cargando..." : "Frase inspiradora o resumen"}
                className="w-full bg-[#090D10] border border-[#1F2E3A] focus:border-[#5BC0BE] focus:ring-1 focus:ring-[#5BC0BE] p-3 text-sm text-white rounded-lg outline-none transition-all h-28 resize-none placeholder:text-white/20"
              />
            </div>
            <div>
              ...
              <div className="aspect-[21/9] bg-[#090D10] rounded-lg overflow-hidden border border-[#1F2E3A] relative group cursor-pointer hover:border-[#5BC0BE]/50 transition-colors">
                {config?.doctorImage ? (
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center text-white/20">
                    {isLoading ? <Loader2 className="animate-spin mb-2" /> : <ImageIcon size={32} />}
                  </div>
                )}
                {!isLoading && (
